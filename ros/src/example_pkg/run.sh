# param 1: hz; 2: ssize, msg size/byte 3: run_time/s; 4: sub num; 5: pub num
# the channel name must be modified manually
rm /ros_test/log/multi/*
rosrun nodelet nodelet manager __name:=nodelet_manager &
for((i=0;i<$4;i++));
do
rosrun nodelet nodelet load example_pkg/B nodelet_manager __name:=B$i _channel_name:=\/A0\/out &
done
sleep 5
