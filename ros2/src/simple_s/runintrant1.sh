...
echo $rate_hz
log_dir='/ros2_test/log/test/'$ps_num't1_10_20_50_100_2i'
rm -rf $log_dir
mkdir $log_dir
...
ros2 run simple_s intra_pipe out 2 $ss 4 $ps_num 1 & #--ros-args --remap __name:=intra_test & # listen to c$i, if listen to the same topic, change to c; how many topics one subscriber listens to(!!!remember to change)
...
do
echo $f
mv /ros2_test/log/test/tmp/$f $log_dir/$f"_"$ps_num"t1_10_20_50_100_2i.csv"
done
done
done
