...
ros2 run simple_s intra_pipe out $sleep_ms $ss 1 1 8 & #--ros-args --remap __name:=intra_test & # listen to c$i, if listen to the same topic, change to c; how many topics one subscriber listens to(!!!remember to change)
...
do
echo $f
mv /ros2_test/log/test/tmp/$f $log_dir/$f"_1t8_2i.csv"
done
done
done
