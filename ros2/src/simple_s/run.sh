...
for((i=0;i<$1;i++));
do
RMW_IMPLEMENTATION=rmw_fastrtps_cpp ros2 run simple_s listener c$i 1 --ros-args --remap __name:=s$i & # listen to c$i, if listen to the same topic, change to c; how many topics one subscriber listens to(!!!remember to change)
# 2 topics example: rosrun simple_s listener c 2 __name:=s$i &
done
sleep 5
for((i=0;i<$2;i++));
do
RMW_IMPLEMENTATION=rmw_fastrtps_cpp ros2 run simple_s talker $3 $4 c$i --ros-args --remap __name:=p$i & # 1: sleep ms; 2: msg size 3: channel name prefix c[number]
done
