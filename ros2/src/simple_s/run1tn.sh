...
for((i=0;i<1;i++));
do
RMW_IMPLEMENTATION=rmw_fastrtps_cpp ros2 run simple_s talker $sleep_ms $ss c --ros-args --remap __name:=p$i & # 1: sleep ms; 2: msg size 3: channel name prefix c[number]
done
