...
log_dir="/apollo/data/log/test/1t4_ci"
mkdir $log_dir
...
for((i=0;i<8;i++));
do
echo $ss\ c$i > /apollo/params$i.txt # param s0,s1... 
done

# launch
cyber_launch start /apollo/cyber/AD_Middle_Test/cyber/simple_s/1tn_t.launch & #./bazel-bin/cyber/AD_Middle_Test/cyber/intra_no_component $2 &
sleep 300
cyber_launch stop
sleep 20
...
do
	...
	mv /apollo/data/log/test/tmp/$f $log_dir/$f"_1t4_ci.csv"
done
done
